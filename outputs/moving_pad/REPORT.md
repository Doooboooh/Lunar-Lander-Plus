...
1. **SB3-PPO 显著最优**（+888，远超另两者）。PPO 的 clipped objective + GAE + 多 epoch
   更新，对"动态目标跟踪"这类需要精细连续控制的任务非常合适；on-policy 的优势在于
   能稳定地优化策略性能。

2. **SB3-DQN 在本任务表现最差**（−196，方差 796）。这与"离散动作 + off-policy"在连续
   精确控制上的固有劣势有关——DQN 的 ε-贪婪探索和 Q 值过估计，使飞船姿态/落点抖动大。
   值得注意的是，DQN 训练曲线虽抖动，但其在某些 seed 下能展现出明显的"左右摇摆找平台"
   行为（见演示 GIF），反而比 PPO 更接近"主动跟踪"的视觉直觉（详见 4.5）。

3. **A2C 中等**：同步策略梯度，性能介于两者之间，稳定性尚可但不如 PPO。
